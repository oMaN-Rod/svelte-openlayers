<script lang="ts">
	...
	import { rotate } from 'ol/transform.js';
...
<div class="container">
	<div>
		Center: [{transformedCenterCoordinates[0].toFixed(2)}, {transformedCenterCoordinates[1].toFixed(2)}]
	</div>
	<div>Zoom: {mapZoom.toFixed(1)}</div>
	<div>Bbox: [{transformedBboxCoordinates[0]?.toFixed(2)}, {transformedBboxCoordinates[1]?.toFixed(2)}, {transformedBboxCoordinates[2]?.toFixed(2)}, {transformedBboxCoordinates[3]?.toFixed(2)}]</div>
</div>
<div>
